"use client";

import i18n from "i18next";
import LanguageDetector from "i18next-browser-languagedetector";
import { initReactI18next } from "react-i18next";

const resources = {
  en: {
    translation: {
      common: {
        appName: "ATLAS",
        language: "Language",
        english: "English",
        hindi: "Hindi",
        spanish: "Spanish",
        french: "French",
        email: "Email",
        password: "Password",
        username: "Username",
      },
      header: {
        closeSidebar: "Close sidebar",
        openSidebar: "Open sidebar",
        closeViewer: "Close viewer",
        openViewer: "Open viewer",
        lightMode: "Light mode",
        darkMode: "Dark mode",
        signOut: "Sign out",
      },
      login: {
        title: "Welcome back",
        description: "Sign in to your Document AI Analyst account",
        fallbackError: "Login failed",
        submit: "Sign In",
        submitting: "Signing in...",
        noAccount: "Don't have an account?",
        createOne: "Create one",
      },
      register: {
        title: "Create Account",
        description: "Start analyzing documents with AI",
        fallbackError: "Registration failed",
        passwordPlaceholder: "Create a strong password",
        submit: "Create Account",
        submitting: "Creating account...",
        hasAccount: "Already have an account?",
        signIn: "Sign in",
        verifyTitle: "Check your email",
        verifyMessage: "We sent a verification link to {{email}}. Verify your email before signing in.",
        openVerification: "Open verification link",
      },
      verifyEmail: {
        title: "Verify Email",
        checking: "Verifying your email...",
        successFallback: "Email verified successfully. You can now log in.",
        errorFallback: "Email verification failed",
        missingToken: "Verification token is missing.",
        signIn: "Sign in",
      },
      password: {
        requirementsTitle: "Password requirements",
        requirementsHelp: "View password requirements",
        showPassword: "Show password",
        hidePassword: "Hide password",
        strengthLabel: "Password strength",
        invalidSubmit: "Please choose a stronger password that meets all requirements.",
        strength: {
          weak: "Weak",
          medium: "Medium",
          strong: "Strong",
        },
        rules: {
          minLength: "At least 8 characters",
          uppercase: "At least one uppercase letter",
          lowercase: "At least one lowercase letter",
          number: "At least one number",
          special: "At least one special character",
          notCommon: "Not a commonly used password",
        },
      },
      chat: {
        askAboutDocument: "Ask about your document",
        selectDocument: "Select a document",
        readyPrompt: "\"{{name}}\" is ready. Ask any question and get cited answers.",
        uploadPrompt: "Upload and select a document from the sidebar to start chatting.",
        fallbackError: "Failed to get response: {{message}}",
        clearConfirm: "Clear all chat history for this document?",
        tool: {
          running: "{{label}}…",
          done: "{{label}} ✓",
          failed: "{{label}} failed",
          generic: "Using {{name}}",
          pdfSearch: "Searching documents",
          webSearch: "Searching the web",
          calculator: "Calculating",
          statistics: "Running statistics",
          codeReview: "Reviewing code",
          readFile: "Reading a file",
          listDirectory: "Listing files",
          fileExists: "Checking a file",
        },
        askPlaceholder: "Ask about \"{{name}}\"...",
        selectPlaceholder: "Select a document first...",
        routingMode: "Response mode",
        modeAuto: "Auto",
        modeQuick: "Quick",
        modeResearch: "Research",
        modeAutoTitle: "Automatically choose the best document workflow",
        modeQuickTitle: "Use loaded documents with the fastest retrieval path",
        modeResearchTitle: "Use deeper retrieval and multi-document synthesis",
        modeCodeReview: "Code Review",
        modeCodeReviewTitle: "Review, debug, or generate code with read-only tools",
        exportTitle: "Export chat history",
        markdown: "Markdown (.md)",
        plainText: "Plain Text (.txt)",
        pdf: "PDF (.pdf)",
        startRecording: "Start recording",
        stopRecording: "Stop recording",
        listening: "Listening... Speak now.",
        stop: "Stop",
        cancelOutput: "Stop response",
        sendMessage: "Send message",
        speechNotSupported: "Speech recognition is not supported in this browser.",
        micPermissionDenied: "Microphone access denied. Please enable permissions in settings.",
        noSpeechDetected: "No speech was detected. Please try again.",
        audioCaptureError: "No microphone found or microphone is not working.",
        networkError: "Network error occurred during speech recognition.",
        speechError: "Speech recognition error: {{message}}",
      },
      documents: {
        uploadFailed: "Upload failed",
        deleteConfirm: "Delete this document and all its data?",
        uploading: "Uploading...",
        dropHere: "Drop files here",
        dropOrClick: "Drop files or click to upload",
        uploadFormats: "PDF, DOCX, TXT, MD (max 50MB)",
        documentsTitle: "Documents ({{count}})",
        noDocuments: "No documents yet",
        getStarted: "Upload a file to get started",
        pagesShort: "{{count}} pg",
        chunks: "{{count}} chunks",
        processing: "Processing",
        failed: "Failed",
        available: "Available",
        noSummary: "No summary available",
        searchableWhileEnriching: "Available for questions while ATLAS finishes enrichment.",
        stages: {
          queued: "Queued",
          extracting: "Extracting text",
          extracting_layout: "Analyzing document layout",
          extracting_ocr: "Reading scanned pages",
          chunking: "Creating sections",
          embedding: "Generating embeddings",
          persisting: "Saving search index",
          searchable: "Available for questions",
          summarizing: "Generating summary",
          building_memory: "Building document memory",
          building_graph: "Mapping document relationships",
          finalizing: "Finalizing metadata",
          completed: "Completed",
          completed_with_warnings: "Completed with warnings",
        },
      },
    },
  },
  hi: {
    translation: {
      common: {
        appName: "डॉक्यूमेंट एआई एनालिस्ट",
        language: "भाषा",
        english: "अंग्रेज़ी",
        hindi: "हिंदी",
        spanish: "स्पेनिश",
        french: "फ़्रेंच",
        email: "ईमेल",
        password: "पासवर्ड",
        username: "उपयोगकर्ता नाम",
      },
      header: {
        closeSidebar: "साइडबार बंद करें",
        openSidebar: "साइडबार खोलें",
        closeViewer: "व्यूअर बंद करें",
        openViewer: "व्यूअर खोलें",
        lightMode: "लाइट मोड",
        darkMode: "डार्क मोड",
        signOut: "साइन आउट",
      },
      login: {
        title: "वापसी पर स्वागत है",
        description: "अपने डॉक्यूमेंट एआई एनालिस्ट खाते में साइन इन करें",
        fallbackError: "लॉगिन विफल",
        submit: "साइन इन करें",
        submitting: "साइन इन हो रहा है...",
        noAccount: "क्या आपका खाता नहीं है?",
        createOne: "एक बनाएं",
      },
      register: {
        title: "खाता बनाएं",
        description: "एआई के साथ दस्तावेज़ों का विश्लेषण शुरू करें",
        fallbackError: "पंजीकरण विफल",
        passwordPlaceholder: "एक मजबूत पासवर्ड बनाएं",
        submit: "खाता बनाएं",
        submitting: "खाता बनाया जा रहा है...",
        hasAccount: "क्या पहले से खाता है?",
        signIn: "साइन इन करें",
        verifyTitle: "अपना ईमेल जांचें",
        verifyMessage: "हमने {{email}} पर सत्यापन लिंक भेजा है। साइन इन करने से पहले अपना ईमेल सत्यापित करें।",
        openVerification: "सत्यापन लिंक खोलें",
      },
      verifyEmail: {
        title: "ईमेल सत्यापित करें",
        checking: "आपका ईमेल सत्यापित हो रहा है...",
        successFallback: "ईमेल सफलतापूर्वक सत्यापित हुआ। अब आप लॉग इन कर सकते हैं।",
        errorFallback: "ईमेल सत्यापन विफल",
        missingToken: "सत्यापन टोकन नहीं मिला।",
        signIn: "साइन इन करें",
      },
      password: {
        requirementsTitle: "पासवर्ड आवश्यकताएँ",
        requirementsHelp: "पासवर्ड आवश्यकताएँ देखें",
        showPassword: "पासवर्ड दिखाएं",
        hidePassword: "पासवर्ड छिपाएं",
        strengthLabel: "पासवर्ड की मजबूती",
        invalidSubmit: "कृपया सभी आवश्यकताओं को पूरा करने वाला मजबूत पासवर्ड चुनें।",
        strength: {
          weak: "कमज़ोर",
          medium: "मध्यम",
          strong: "मजबूत",
        },
        rules: {
          minLength: "कम से कम 8 अक्षर",
          uppercase: "कम से कम एक बड़ा अक्षर",
          lowercase: "कम से कम एक छोटा अक्षर",
          number: "कम से कम एक संख्या",
          special: "कम से कम एक विशेष वर्ण",
          notCommon: "आमतौर पर इस्तेमाल किया जाने वाला पासवर्ड नहीं",
        },
      },
      chat: {
        askAboutDocument: "अपने दस्तावेज़ के बारे में पूछें",
        selectDocument: "एक दस्तावेज़ चुनें",
        readyPrompt: "\"{{name}}\" तैयार है। कोई भी प्रश्न पूछें और स्रोत सहित उत्तर पाएं।",
        uploadPrompt: "चैट शुरू करने के लिए साइडबार से फ़ाइल अपलोड और चुनें।",
        fallbackError: "जवाब प्राप्त नहीं हुआ: {{message}}",
        clearConfirm: "क्या इस दस्तावेज़ का पूरा चैट इतिहास साफ़ करें?",
        tool: {
          running: "{{label}}…",
          done: "{{label}} ✓",
          failed: "{{label}} विफल",
          generic: "{{name}} का उपयोग हो रहा है",
          pdfSearch: "दस्तावेज़ों में खोज हो रही है",
          webSearch: "वेब पर खोज हो रही है",
          calculator: "गणना हो रही है",
          statistics: "सांख्यिकी चल रही है",
          codeReview: "कोड की समीक्षा हो रही है",
          readFile: "फ़ाइल पढ़ी जा रही है",
          listDirectory: "फ़ाइलें सूचीबद्ध हो रही हैं",
          fileExists: "फ़ाइल जांची जा रही है",
        },
        askPlaceholder: "\"{{name}}\" के बारे में पूछें...",
        selectPlaceholder: "पहले एक दस्तावेज़ चुनें...",
        routingMode: "उत्तर मोड",
        modeAuto: "ऑटो",
        modeQuick: "त्वरित",
        modeResearch: "अनुसंधान",
        modeAutoTitle: "सबसे उपयुक्त दस्तावेज़ कार्यप्रवाह स्वतः चुनें",
        modeQuickTitle: "लोड किए गए दस्तावेज़ों से तेज़ उत्तर दें",
        modeResearchTitle: "गहन खोज और बहु-दस्तावेज़ संश्लेषण का उपयोग करें",
        modeCodeReview: "कोड समीक्षा",
        modeCodeReviewTitle: "केवल-पठन उपकरणों से कोड की समीक्षा, डिबग या जनरेट करें",
        exportTitle: "चैट इतिहास निर्यात करें",
        markdown: "मार्कडाउन (.md)",
        plainText: "सादा पाठ (.txt)",
        pdf: "पीडीएफ (.pdf)",
        startRecording: "रिकॉर्डिंग शुरू करें",
        stopRecording: "रिकॉर्डिंग बंद करें",
        listening: "सुन रहा हूँ... अब बोलिए।",
        stop: "रोकें",
        cancelOutput: "उत्तर रोकें",
        sendMessage: "संदेश भेजें",
        speechNotSupported: "इस ब्राउज़र में स्पीच रिकग्निशन समर्थित नहीं है।",
        micPermissionDenied: "माइक्रोफ़ोन एक्सेस अस्वीकृत। कृपया सेटिंग में अनुमति चालू करें।",
        noSpeechDetected: "कोई आवाज़ नहीं सुनी गई। कृपया पुनः प्रयास करें।",
        audioCaptureError: "कोई माइक्रोफ़ोन नहीं मिला या माइक्रोफ़ोन काम नहीं कर रहा है।",
        networkError: "स्पीच रिकग्निशन के दौरान नेटवर्क त्रुटि हुई।",
        speechError: "स्पीच रिकग्निशन त्रुटि: {{message}}",
      },
      documents: {
        uploadFailed: "अपलोड विफल",
        deleteConfirm: "क्या इस दस्तावेज़ और उसके सभी डेटा को हटाएं?",
        uploading: "अपलोड हो रहा है...",
        dropHere: "फ़ाइलें यहाँ छोड़ें",
        dropOrClick: "फ़ाइलें छोड़ें या अपलोड के लिए क्लिक करें",
        uploadFormats: "PDF, DOCX, TXT, MD (अधिकतम 50MB)",
        documentsTitle: "दस्तावेज़ ({{count}})",
        noDocuments: "अभी तक कोई दस्तावेज़ नहीं",
        getStarted: "शुरू करने के लिए फ़ाइल अपलोड करें",
        pagesShort: "{{count}} पेज",
        chunks: "{{count}} खंड",
        processing: "प्रोसेस हो रहा है",
        failed: "विफल",
        available: "उपलब्ध",
        noSummary: "सारांश उपलब्ध नहीं है",
        searchableWhileEnriching: "ATLAS के संवर्धन पूरा करने के दौरान प्रश्न पूछे जा सकते हैं।",
        stages: {
          queued: "कतार में",
          extracting: "पाठ निकाला जा रहा है",
          extracting_layout: "दस्तावेज़ संरचना का विश्लेषण",
          extracting_ocr: "स्कैन किए पृष्ठ पढ़े जा रहे हैं",
          chunking: "खंड बनाए जा रहे हैं",
          embedding: "एम्बेडिंग बनाई जा रही हैं",
          persisting: "खोज सूचकांक सहेजा जा रहा है",
          searchable: "प्रश्नों के लिए उपलब्ध",
          summarizing: "सारांश बनाया जा रहा है",
          building_memory: "दस्तावेज़ स्मृति बनाई जा रही है",
          building_graph: "संबंध मैप किए जा रहे हैं",
          finalizing: "अंतिम रूप दिया जा रहा है",
          completed: "पूर्ण",
          completed_with_warnings: "चेतावनियों के साथ पूर्ण",
        },
      },
    },
  },
  es: {
    translation: {
      common: {
        appName: "Analista IA de Documentos",
        language: "Idioma",
        english: "Inglés",
        hindi: "Hindi",
        spanish: "Español",
        french: "Francés",
        email: "Correo electrónico",
        password: "Contraseña",
        username: "Nombre de usuario",
      },
      header: {
        closeSidebar: "Cerrar barra lateral",
        openSidebar: "Abrir barra lateral",
        closeViewer: "Cerrar visor",
        openViewer: "Abrir visor",
        lightMode: "Modo claro",
        darkMode: "Modo oscuro",
        signOut: "Cerrar sesión",
      },
      login: {
        title: "Bienvenido de nuevo",
        description: "Inicia sesión en tu cuenta de Analista IA de Documentos",
        fallbackError: "Error al iniciar sesión",
        submit: "Iniciar sesión",
        submitting: "Iniciando sesión...",
        noAccount: "¿No tienes una cuenta?",
        createOne: "Crear una",
      },
      register: {
        title: "Crear cuenta",
        description: "Empieza a analizar documentos con IA",
        fallbackError: "Error de registro",
        passwordPlaceholder: "Crea una contraseña segura",
        submit: "Crear cuenta",
        submitting: "Creando cuenta...",
        hasAccount: "¿Ya tienes una cuenta?",
        signIn: "Inicia sesión",
        verifyTitle: "Revisa tu correo",
        verifyMessage: "Enviamos un enlace de verificación a {{email}}. Verifica tu correo antes de iniciar sesión.",
        openVerification: "Abrir enlace de verificación",
      },
      verifyEmail: {
        title: "Verificar correo",
        checking: "Verificando tu correo...",
        successFallback: "Correo verificado correctamente. Ya puedes iniciar sesión.",
        errorFallback: "La verificación del correo falló",
        missingToken: "Falta el token de verificación.",
        signIn: "Iniciar sesión",
      },
      password: {
        requirementsTitle: "Requisitos de contraseña",
        requirementsHelp: "Ver requisitos de contraseña",
        showPassword: "Mostrar contraseña",
        hidePassword: "Ocultar contraseña",
        strengthLabel: "Fortaleza de la contraseña",
        invalidSubmit: "Elige una contraseña más segura que cumpla todos los requisitos.",
        strength: {
          weak: "Débil",
          medium: "Media",
          strong: "Fuerte",
        },
        rules: {
          minLength: "Al menos 8 caracteres",
          uppercase: "Al menos una letra mayúscula",
          lowercase: "Al menos una letra minúscula",
          number: "Al menos un número",
          special: "Al menos un carácter especial",
          notCommon: "No es una contraseña común",
        },
      },
      chat: {
        askAboutDocument: "Pregunta sobre tu documento",
        selectDocument: "Selecciona un documento",
        readyPrompt: "\"{{name}}\" está listo. Haz cualquier pregunta y obtén respuestas con citas.",
        uploadPrompt: "Sube y selecciona un documento de la barra lateral para comenzar a chatear.",
        fallbackError: "No se pudo obtener respuesta: {{message}}",
        clearConfirm: "¿Borrar todo el historial de chat de este documento?",
        tool: {
          running: "{{label}}…",
          done: "{{label}} ✓",
          failed: "{{label}} falló",
          generic: "Usando {{name}}",
          pdfSearch: "Buscando en los documentos",
          webSearch: "Buscando en la web",
          calculator: "Calculando",
          statistics: "Ejecutando estadísticas",
          codeReview: "Revisando código",
          readFile: "Leyendo un archivo",
          listDirectory: "Listando archivos",
          fileExists: "Comprobando un archivo",
        },
        askPlaceholder: "Pregunta sobre \"{{name}}\"...",
        selectPlaceholder: "Primero selecciona un documento...",
        routingMode: "Modo de respuesta",
        modeAuto: "Auto",
        modeQuick: "Rápido",
        modeResearch: "Investigación",
        modeAutoTitle: "Elegir automáticamente el flujo documental más adecuado",
        modeQuickTitle: "Responder con los documentos cargados mediante la ruta más rápida",
        modeResearchTitle: "Usar recuperación profunda y síntesis multidocumento",
        modeCodeReview: "Revisión de Código",
        modeCodeReviewTitle: "Revisar, depurar o generar código con herramientas de solo lectura",
        exportTitle: "Exportar historial del chat",
        markdown: "Markdown (.md)",
        plainText: "Texto plano (.txt)",
        pdf: "PDF (.pdf)",
        startRecording: "Iniciar grabación",
        stopRecording: "Detener grabación",
        listening: "Escuchando... Hable ahora.",
        stop: "Detener",
        cancelOutput: "Detener respuesta",
        sendMessage: "Enviar mensaje",
        speechNotSupported: "El reconocimiento de voz no es compatible con este navegador.",
        micPermissionDenied: "Acceso al micrófono denegado. Habilite los permisos en la configuración.",
        noSpeechDetected: "No se detectó voz. Por favor, inténtelo de nuevo.",
        audioCaptureError: "No se encontró ningún micrófono o no está funcionando.",
        networkError: "Ocurrió un error de red durante el reconocimiento de voz.",
        speechError: "Error de reconocimiento de voz: {{message}}",
      },
      documents: {
        uploadFailed: "Error de carga",
        deleteConfirm: "¿Eliminar este documento y todos sus datos?",
        uploading: "Subiendo...",
        dropHere: "Suelta archivos aquí",
        dropOrClick: "Suelta archivos o haz clic para subir",
        uploadFormats: "PDF, DOCX, TXT, MD (máx. 50MB)",
        documentsTitle: "Documentos ({{count}})",
        noDocuments: "Aún no hay documentos",
        getStarted: "Sube un archivo para comenzar",
        pagesShort: "{{count}} pág",
        chunks: "{{count}} fragmentos",
        processing: "Procesando",
        failed: "Falló",
        available: "Disponible",
        noSummary: "No hay resumen disponible",
        searchableWhileEnriching: "Disponible para preguntas mientras ATLAS completa el enriquecimiento.",
        stages: {
          queued: "En cola",
          extracting: "Extrayendo texto",
          extracting_layout: "Analizando la estructura del documento",
          extracting_ocr: "Leyendo páginas escaneadas",
          chunking: "Creando secciones",
          embedding: "Generando embeddings",
          persisting: "Guardando el índice de búsqueda",
          searchable: "Disponible para preguntas",
          summarizing: "Generando resumen",
          building_memory: "Construyendo memoria documental",
          building_graph: "Mapeando relaciones del documento",
          finalizing: "Finalizando metadatos",
          completed: "Completado",
          completed_with_warnings: "Completado con advertencias",
        },
      },
    },
  },
  fr: {
    translation: {
      common: {
        appName: "Analyste IA de Documents",
        language: "Langue",
        english: "Anglais",
        hindi: "Hindi",
        spanish: "Espagnol",
        french: "Français",
        email: "E-mail",
        password: "Mot de passe",
        username: "Nom d'utilisateur",
      },
      header: {
        closeSidebar: "Fermer la barre latérale",
        openSidebar: "Ouvrir la barre latérale",
        closeViewer: "Fermer le lecteur",
        openViewer: "Ouvrir le lecteur",
        lightMode: "Mode clair",
        darkMode: "Mode sombre",
        signOut: "Se déconnecter",
      },
      login: {
        title: "Bon retour",
        description: "Connectez-vous à votre compte Analyste IA de Documents",
        fallbackError: "Échec de la connexion",
        submit: "Se connecter",
        submitting: "Connexion en cours...",
        noAccount: "Vous n'avez pas de compte ?",
        createOne: "En créer un",
      },
      register: {
        title: "Créer un compte",
        description: "Commencez à analyser des documents avec l'IA",
        fallbackError: "Échec de l'inscription",
        passwordPlaceholder: "Créez un mot de passe fort",
        submit: "Créer un compte",
        submitting: "Création du compte...",
        hasAccount: "Vous avez déjà un compte ?",
        signIn: "Se connecter",
        verifyTitle: "Consultez votre e-mail",
        verifyMessage: "Nous avons envoyé un lien de vérification à {{email}}. Vérifiez votre e-mail avant de vous connecter.",
        openVerification: "Ouvrir le lien de vérification",
      },
      verifyEmail: {
        title: "Vérifier l'e-mail",
        checking: "Vérification de votre e-mail...",
        successFallback: "E-mail vérifié avec succès. Vous pouvez maintenant vous connecter.",
        errorFallback: "La vérification de l'e-mail a échoué",
        missingToken: "Le jeton de vérification est manquant.",
        signIn: "Se connecter",
      },
      password: {
        requirementsTitle: "Exigences du mot de passe",
        requirementsHelp: "Voir les exigences du mot de passe",
        showPassword: "Afficher le mot de passe",
        hidePassword: "Masquer le mot de passe",
        strengthLabel: "Force du mot de passe",
        invalidSubmit: "Veuillez choisir un mot de passe plus fort qui respecte toutes les exigences.",
        strength: {
          weak: "Faible",
          medium: "Moyen",
          strong: "Fort",
        },
        rules: {
          minLength: "Au moins 8 caractères",
          uppercase: "Au moins une lettre majuscule",
          lowercase: "Au moins une lettre minuscule",
          number: "Au moins un chiffre",
          special: "Au moins un caractère spécial",
          notCommon: "Pas un mot de passe couramment utilisé",
        },
      },
      chat: {
        askAboutDocument: "Posez une question sur votre document",
        selectDocument: "Sélectionnez un document",
        readyPrompt: "\"{{name}}\" est prêt. Posez n'importe quelle question et obtenez des réponses sourcées.",
        uploadPrompt: "Importez puis sélectionnez un document dans la barre latérale pour commencer à discuter.",
        fallbackError: "Impossible d'obtenir une réponse : {{message}}",
        clearConfirm: "Effacer tout l'historique de discussion de ce document ?",
        tool: {
          running: "{{label}}…",
          done: "{{label}} ✓",
          failed: "Échec : {{label}}",
          generic: "Utilisation de {{name}}",
          pdfSearch: "Recherche dans les documents",
          webSearch: "Recherche sur le web",
          calculator: "Calcul en cours",
          statistics: "Analyse statistique en cours",
          codeReview: "Revue de code en cours",
          readFile: "Lecture d'un fichier",
          listDirectory: "Liste des fichiers",
          fileExists: "Vérification d'un fichier",
        },
        askPlaceholder: "Posez une question sur \"{{name}}\"...",
        selectPlaceholder: "Sélectionnez d'abord un document...",
        routingMode: "Mode de réponse",
        modeAuto: "Auto",
        modeQuick: "Rapide",
        modeResearch: "Recherche",
        modeAutoTitle: "Choisir automatiquement le meilleur flux documentaire",
        modeQuickTitle: "Répondre rapidement à partir des documents chargés",
        modeResearchTitle: "Utiliser une recherche approfondie et une synthèse multidocument",
        modeCodeReview: "Revue de Code",
        modeCodeReviewTitle: "Revoir, déboguer ou générer du code avec des outils en lecture seule",
        exportTitle: "Exporter l'historique du chat",
        markdown: "Markdown (.md)",
        plainText: "Texte brut (.txt)",
        pdf: "PDF (.pdf)",
        startRecording: "Démarrer l'enregistrement",
        stopRecording: "Arrêter l'enregistrement",
        listening: "Écoute en cours... Parlez maintenant.",
        stop: "Arrêter",
        cancelOutput: "Arrêter la réponse",
        sendMessage: "Envoyer le message",
        speechNotSupported: "La reconnaissance vocale n'est pas prise en charge par ce navigateur.",
        micPermissionDenied: "Accès au microphone refusé. Veuillez activer les autorisations dans les paramètres.",
        noSpeechDetected: "Aucune parole n'a été détectée. Veuillez réessayer.",
        audioCaptureError: "Aucun microphone trouvé ou le microphone ne fonctionne pas.",
        networkError: "Une erreur réseau s'est produite lors de la reconnaissance vocale.",
        speechError: "Erreur de reconnaissance vocale : {{message}}",
      },
      documents: {
        uploadFailed: "Échec de l'envoi",
        deleteConfirm: "Supprimer ce document et toutes ses données ?",
        uploading: "Envoi en cours...",
        dropHere: "Déposez les fichiers ici",
        dropOrClick: "Déposez les fichiers ou cliquez pour téléverser",
        uploadFormats: "PDF, DOCX, TXT, MD (max 50 Mo)",
        documentsTitle: "Documents ({{count}})",
        noDocuments: "Aucun document pour le moment",
        getStarted: "Importez un fichier pour commencer",
        pagesShort: "{{count}} p",
        chunks: "{{count}} segments",
        processing: "Traitement",
        failed: "Échec",
        available: "Disponible",
        noSummary: "Aucun résumé disponible",
        searchableWhileEnriching: "Disponible pour les questions pendant qu’ATLAS termine l’enrichissement.",
        stages: {
          queued: "En attente",
          extracting: "Extraction du texte",
          extracting_layout: "Analyse de la structure du document",
          extracting_ocr: "Lecture des pages numérisées",
          chunking: "Création des sections",
          embedding: "Génération des embeddings",
          persisting: "Enregistrement de l’index de recherche",
          searchable: "Disponible pour les questions",
          summarizing: "Génération du résumé",
          building_memory: "Construction de la mémoire documentaire",
          building_graph: "Cartographie des relations",
          finalizing: "Finalisation des métadonnées",
          completed: "Terminé",
          completed_with_warnings: "Terminé avec avertissements",
        },
      },
    },
  },
} as const;

if (!i18n.isInitialized) {
  void i18n
    .use(LanguageDetector)
    .use(initReactI18next)
    .init({
      resources,
      fallbackLng: "en",
      supportedLngs: ["en", "hi", "es", "fr"],
      interpolation: {
        escapeValue: false,
      },
      detection: {
        order: ["localStorage", "navigator"],
        caches: ["localStorage"],
        lookupLocalStorage: "i18nextLng",
      },
      react: {
        useSuspense: false,
      },
    });
}

export default i18n;
