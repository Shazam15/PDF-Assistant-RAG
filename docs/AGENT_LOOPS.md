...
3. **Herramienta requerida explícitamente**: `_required_tool` busca frases fijas en categorías —web, cálculo, código y, solo si hay al menos un servidor MCP configurado, "archivos" (p. ej. "lista los archivos", "list the files")— sobre la pregunta normalizada; ante una coincidencia → `tool_agent` con `required_tool` fijado.
...
`build_agent_tools(user_id, document_id, top_k)` construye la lista completa: los cinco internos —`PDFSearchTool` (`pdf_search`), `CodeReviewTool` (`code_review`), `MathTool` (`calculator`), `WebSearchTool` (`web_search`), `StatisticsTool` (`statistics`)— más `load_mcp_tools()` (herramientas MCP descubiertas en tiempo de ejecución, ver §6). Es la misma lista, y el mismo `AgentExecutor`, el que despacha herramientas internas y MCP: no hay una ruta de despacho separada para MCP dentro del bucle del agente.
...
## 6. Herramientas MCP (Model Context Protocol)
...
## 7. Cancelación y llamadas a herramientas en curso
