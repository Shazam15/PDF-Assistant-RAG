...
`RERANK_MAX_LENGTH` (2048 por defecto, dimensionado para Qwen3-Reranker) se ajusta automáticamente al `max_position_embeddings` real del modelo cargado (`Reranker._safe_max_length`): si el valor configurado excede lo que el modelo puede indexar, se recorta a `max_position_embeddings - 8` en vez de dejar que el *forward pass* falle con un `IndexError` críptico a mitad de la inferencia — relevante al cambiar `RERANKER_MODEL` a uno con una ventana de contexto menor sin ajustar `RERANK_MAX_LENGTH` a mano.
